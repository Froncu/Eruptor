...
#include <eruptor/core/window.hpp>
...
namespace eru
{
   ...
   auto Context::create_buffer(vk::BufferCreateInfo const& create_info) const -> vk::raii::Buffer
   {
      vk::ResultValue buffer{ device.createBuffer(create_info) };
      RUNTIME_ASSERT(buffer.has_value(),
         std::format("failed to create buffer! ({})", to_string(buffer.result)));
   ...
   auto Context::create_fences(std::uint32_t count) const -> std::vector<vk::raii::Fence>
   {
   ...
   auto Context::create_instance() const -> vk::raii::Instance
   {
      ...
      RUNTIME_ASSERT(result.has_value(),
         std::format("failed to create a Vulkan instance! ({})", to_string(result.result)));
   ...
   auto Context::create_debug_messenger() const -> vk::raii::DebugUtilsMessengerEXT
   {
      ...
      RUNTIME_ASSERT(result.has_value(),
         std::format("failed to create a debug messenger! ({})", to_string(result.result)));
   ...
   auto Context::pick_physical_device() const -> vk::raii::PhysicalDevice
   {
      ...
      auto const picked_physical_device{
         std::ranges::find_if(
            *physical_devices,
            [](vk::raii::PhysicalDevice const& device)
            {
               ...
               return properties.get().properties.deviceType == vk::PhysicalDeviceType::eDiscreteGpu
                  ...
                  and features.get<vk::PhysicalDeviceVulkan12Features>().scalarBlockLayout;
            })
      };

      RUNTIME_ASSERT(picked_physical_device not_eq std::ranges::end(*physical_devices),
   ...
   auto Context::create_device() const -> vk::raii::Device
   {
      ...
      RUNTIME_ASSERT(result.has_value(),
         std::format("failed to create a device! ({})", to_string(result.result)));
   ...
   auto Context::create_command_pool() const -> vk::raii::CommandPool
   {
      ...
      RUNTIME_ASSERT(result.has_value(),
         std::format("failed create a command pool! ({})", to_string(result.result)));
