...
#include <eruptor/core/swap_chain.hpp>
   ...
   auto SwapChain::swap_chain(Description const& parameters) const -> vk::raii::SwapchainKHR
   {
      ...
      if (surface_capabilities->currentExtent.width == std::numeric_limits<std::uint32_t>::max() and surface_capabilities->currentExtent.height == std::numeric_limits<std::uint32_t>::max())
         surface_capabilities->currentExtent = {
            .width{ std::clamp<std::uint32_t>(parameters.extent.width, surface_capabilities->minImageExtent.width, surface_capabilities->maxImageExtent.width) },
            .height{ std::clamp<std::uint32_t>(parameters.extent.height, surface_capabilities->minImageExtent.height, surface_capabilities->maxImageExtent.height) }
         };
